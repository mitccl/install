...
Example:
<content>plugin://script.metalgearsolid?info=discography&amp;artistname=INSERT_ARTIST_NAME_HERE</content>
- keep Attention to the parameter separators: "&amp;" some places it may need to be "&"
Make your own custom commands:
- RunScript(script.metalgearsolid,argument1=value1,argument2=value2)
- RunPlugin(plugin://script.metalgearsolid/?argument1=value1&argument2=value2)
- ActivateWindow(Videos,plugin://script.metalgearsolid/?argument1=value1&argument2=value2,return)

### Custom (added by OpenELEQ)

RunScript(script.metalgearsolid,info=list,type=movie)     --> Starts script with movie-type
...
RunScript(script.metalgearsolid,info=list,type=tv)        --> Starts script with tv-type
  - optional additional parameter: query=
    - set query to "qqqqq" to force keyboard
  - optional additional parameter: iquery=              --> Executes custom iSearch using tv-list (~/resources/extras/tv/favourites.xml)
    - set iquery to "qqqqq" to force keyboard

RunScript(script.metalgearsolid,info=list,iquery=)        --> Executes custom iSearch using total list (~/resources/extras/total/favourites.xml)
  - set iquery to "qqqqq" to force keyboard

RunScript(script.metalgearsolid,info=list,type=channel)   --> Starts youtube-browser with channel-type
  - optional additional parameter: query=

RunScript(script.metalgearsolid,info=list,type=playlist)  --> Starts youtube-browser with playlist-type
  - optional additional parameter: query=

RunScript(script.metalgearsolid,info=list,type=video)     --> Starts youtube-browser with video-type
  - optional additional parameter: query=

RunScript(script.metalgearsolid,info=list,listid=)        --> Starts script using listid
  - TMDb login required

RunScript(script.metalgearsolid,info=string)              --> Sets SkinString (TotalSearch) and refreshes container
...
RunScript(script.metalgearsolid,info=zmovies)             --> All Movies in Script GUI
  - optional additional parameter: query=
...
RunScript(script.metalgearsolid,info=incinemas)           --> InCinemasMovies.%d
RunScript(script.metalgearsolid,info=upcoming)            --> UpcomingMovies.%d
RunScript(script.metalgearsolid,info=popularmovies)       --> PopularMovies.%d
RunScript(script.metalgearsolid,info=topratedmovies)      --> TopRatedMovies.%d
RunScript(script.metalgearsolid,info=similarmovies)       --> SimilarMovies.%d
  - required additional parameters: dbid=
RunScript(script.metalgearsolid,info=set)                 --> MovieSetItems.%d
- fetches a list of movies from the same Set
  - required additional parameters: dbid=
RunScript(script.metalgearsolid,info=directormovies)      --> DirectorMovies.%d
  - required additional parameters: director=
RunScript(script.metalgearsolid,info=writermovies)        --> WriterMovies.%d
  - required additional parameters: writer=
RunScript(script.metalgearsolid,info=studio)              --> StudioInfo.%d
...
RunScript(script.metalgearsolid,info=ztvshows)            --> All TVShows in Script GUI
  - optional additional parameter: query=
...
RunScript(script.metalgearsolid,info=populartvshows)      --> PopularTVShows.%d
RunScript(script.metalgearsolid,info=topratedtvshows)     --> TopRatedTVShows.%d
RunScript(script.metalgearsolid,info=onairtvshows)        --> OnAirTVShows.%d
RunScript(script.metalgearsolid,info=airingtodaytvshows)  --> AiringTodayTVShows.%d
...
### Trakt.tv
RunScript(script.metalgearsolid,info=trendingmovies)      --> TrendingMovies.%d
RunScript(script.metalgearsolid,info=similarmoviestrakt)  --> SimilarMovies.%d
...
RunScript(script.metalgearsolid,info=trendingshows)         --> TrendingShows.%d
RunScript(script.metalgearsolid,info=similartvshowstrakt)   --> SimilarTVShows.%d
...
RunScript(script.metalgearsolid,info=airingshows)         --> AiringShows.%d
RunScript(script.metalgearsolid,info=premiereshows)       --> PremiereShows.%d
...
### TheAudioDB
RunScript(script.metalgearsolid,info=discography)         --> Discography.%d
...
RunScript(script.metalgearsolid,info=mostlovedtracks)         --> MostLovedTracks.%d
- fetches most loved tracks for the given artist (TheAudioDB)
  - required additional parameters: artistname=
RunScript(script.metalgearsolid,info=albuminfo)               --> TrackInfo.%d
...
RunScript(script.metalgearsolid,info=artistdetails) ???

### LastFM
RunScript(script.metalgearsolid,info=albumshouts)
- fetches twitter shouts for given album
  - required additional parameters: artistname=, albumname=
RunScript(script.metalgearsolid,info=artistshouts)
...
RunScript(script.metalgearsolid,info=topartists)
- fetches a lists of the most popular artists
RunScript(script.metalgearsolid,info=hypedartists)
...
RunScript(script.metalgearsolid,info=nearevents)       --> NearEvents.%d
...
### YouTube
RunScript(script.metalgearsolid,info=youtubesearch)           --> YoutubeSearch.%d
  - required additional parameters: id=
RunScript(script.metalgearsolid,info=youtubeplaylist)         --> YoutubePlaylist.%d
  - required additional parameters: id=
RunScript(script.metalgearsolid,info=youtubeusersearch)       --> YoutubeUserSearch.%d
  - required additional parameters: id=
...
### Misc Images
RunScript(script.metalgearsolid,info=xkcd)                   --> XKCD.%d
- fetches a daily random list of XKCD webcomics
RunScript(script.metalgearsolid,info=cyanide)                --> CyanideHappiness.%d
- fetches a daily random list of Cyanide & Happiness webcomics
RunScript(script.metalgearsolid,info=dailybabe)              --> DailyBabe.%d
RunScript(script.metalgearsolid,info=dailybabes)             --> DailyBabes.%d
...
RunScript(script.metalgearsolid,info=extendedactorinfo,name=ACTORNAME)
RunScript(script.metalgearsolid,info=extendedactorinfo,id=ACTOR_TMDB_ID)

possible script calls for Movie Info Dialog:
RunScript(script.metalgearsolid,info=metalgearsolid,name=MOVIENAME)
RunScript(script.metalgearsolid,info=metalgearsolid,id=MOVIE_TMDB_ID)
RunScript(script.metalgearsolid,info=metalgearsolid,dbid=MOVIE_DBID)
RunScript(script.metalgearsolid,info=metalgearsolid,imdb_id=IMDB_ID)
...
#### List of Built In Controls for add-on dialogs :
 - MOVIES, TVSHOWS, SEASONS, EPISODES: script.metalgearsolid-DialogVideoInfo.xml
 - ACTORS: script.metalgearsolid-DialogInfo.xml
...
#### Labels Available In script.metalgearsolid-DialogInfo.xml:
...
#### Labels Available In script.metalgearsolid-DialogVideoInfo.xml:
