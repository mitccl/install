    ...
    class DialogVideoList(DialogBaseList, window_type):
        ...
        def context_menu(self):
                ...
                if self.type == "tv" or self.type == "episode":
                    if self.listitem.getProperty("dbid"):
                        ...
                        if os.path.exists(show_playlist):
                            url = show_playlist
                        else: url = 'plugin://plugin.video.metalgear/tv/play/%s/1/1/%s' % (tvdb_id, SETTING("player_alt"))
                    else: url = 'plugin://plugin.video.metalgear/tv/play/%s/1/1/%s' % (tvdb_id, SETTING("player_alt"))
                else:
                    if self.listitem.getProperty("dbid"): url = 'temp'
                    else: url = 'plugin://plugin.video.metalgear/movies/play/tmdb/%s/%s' % (item_id, SETTING("player_alt"))
            ...
            if selection == 1:
                ...
                self.getControl(500).selectItem(self.position)
            if selection == 2:
                if self.type == "tv": url = "plugin://script.metalgearsolid/?info=playtvtrailer&id=%s" % item_id
                elif self.type == "episode": url = "plugin://script.metalgearsolid/?info=playtvtrailer&tvdb_id=%s" % tvdb_id
                elif self.type == "movie": url = "plugin://script.metalgearsolid/?info=playtrailer&id=%s" % item_id
            ...
            if selection == 3:
                if self.logged_in:
                    ...
                    if self.type == "episode":
                        path = xbmc.translatePath("special://profile/addon_data/script.metalgearsolid/exclusions_tvdb.txt")
