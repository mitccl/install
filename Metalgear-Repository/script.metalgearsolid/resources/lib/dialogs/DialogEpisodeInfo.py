...
def get_episode_window(window_type):
    ...
    class DialogEpisodeInfo(DialogBaseInfo, window_type):
        ...
        def play_episode_no_resume(self):
            if self.dbid:
                ...
                if not os.path.exists(url):
                    url = "plugin://plugin.video.metalgear/tv/play/%s/%s/%s/%s" % (self.info.get("tvdb_id", ""), self.info.get("season", ""), self.info["episode"], SETTING("player_main_tv"))
            else:
                dbid = 0
                tvdb_id = fetch(get_tvshow_ids(self.tvshow_id), "tvdb_id")
                url = "plugin://plugin.video.metalgear/tv/play/%s/%s/%s/%s" % (tvdb_id, self.info["season"], self.info["episode"], SETTING("player_main_tv"))
        ...
        def show_manage_dialog(self):
            manage_list = []
            manage_list.append([xbmc.getInfoLabel('System.AddonTitle(%s)' % 'script.metalgearsolid') + " " + LANG(32133), 'Addon.OpenSettings(%s)' % 'script.metalgearsolid'])
            manage_list.append([xbmc.getInfoLabel('System.AddonTitle(%s)' % 'plugin.video.metalgear') + " " + LANG(32133), 'Addon.OpenSettings(%s)' % 'plugin.video.metalgear'])
        ...
        def pick_and_mix(self):
            url = 'plugin://plugin.video.metalgear/tv/play/%s/%s/%s/%s' % (self.data.get("tvdb_id", ""), self.info.get("season", ""), self.info.get("episode", ""), self.listitem.getProperty("qid"))
