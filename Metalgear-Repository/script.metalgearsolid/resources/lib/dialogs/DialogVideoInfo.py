...
def get_movie_window(window_type):
    ...
    class DialogVideoInfo(DialogBaseInfo, window_type):
        ...
        def add_list_to_account(self):
            ...
            if selection == -1:
                ...
                for item in movies_list:
                    if item != "":
                        ids = ids + str(item[1]) + '\n'
                batch_add_file_path = "special://profile/addon_data/plugin.video.metalgear/movies_to_add.txt"
                ...
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.metalgear/movies/batch_add_to_library)")
                xbmc.executebuiltin("Dialog.Close(busydialog)")
            elif selection > 0:
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.metalgear/movies/add_to_library/tmdb/%s)" % str(movies_list[selection][1]))
        ...
        def search_in_metalliq_by_title(self):
            url = "plugin://plugin.video.metalgear/movies/tmdb/search_term/%s/1" % self.info.get("title", "")
            self.close()
        ...
        def play_movie_no_resume(self):
            if self.dbid:
                ...
                dbid = 0
                url = "plugin://plugin.video.metalgear/movies/play/tmdb/%s/%s" % (self.info.get("id", ""), SETTING("player_main_movie"))
        ...
        def show_manage_dialog(self):
            ...
            manage_list.append([xbmc.getInfoLabel('System.AddonTitle(%s)' % 'plugin.video.metalgear') + " " + LANG(10004), 'Addon.OpenSettings(%s)' % 'plugin.video.metalgear'])
        ...
        def appintegration_movie(self):
            ...
            if xbmc.getCondVisibility("system.hasaddon(script.metalgearsolid)"):
                manage_list.append(["Search Youtube Videos", "RunScript(script.metalgearsolid,info=list,type=video,query="+title+")||Notification(YouTube Videos:,"+title+",5000,special://home/addons/script.metalgearsolid/resources/skins/Default/media/common/youtube.png)"])
            if xbmc.getCondVisibility("system.hasaddon(script.metalgearsolid)") and ADDON.getSetting("ExtendedYoutubeMovie") == "true":
                manage_list.append(["Search Youtube Channels", "RunScript(script.metalgearsolid,info=list,type=channel,query="+title+")||Notification(YouTube Channels:,"+title+",5000,special://home/addons/script.metalgearsolid/resources/skins/Default/media/common/youtube.png)"])
            if xbmc.getCondVisibility("system.hasaddon(script.metalgearsolid)") and ADDON.getSetting("ExtendedYoutubeMovie") == "true":
                manage_list.append(["Search Youtube Playlists", "RunScript(script.metalgearsolid,info=list,type=playlist,query="+title+")||Notification(YouTube Playlists:,"+title+",5000,special://home/addons/script.metalgearsolid/resources/skins/Default/media/common/youtube.png)"])
        ...
        def pick_and_mix(self):
            dbid = 0
            url = "plugin://plugin.video.metalgear/movies/play/tmdb/%s/%s" % (self.info.get("id", ""), self.listitem.getProperty("qid"))
        ...
        def add_movie_to_library(self):
            ...
            if not os.path.exists(xbmc.translatePath("%s%s/" % (MovieLibrary, imdb_id))):
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.metalgear/movies/add_to_library/tmdb/%s)" % self.info.get("id", ""))
