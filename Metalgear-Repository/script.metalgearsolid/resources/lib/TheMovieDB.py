...
def handle_tmdb_movies(results=[], local_first=True, sortkey="year"):
    ...
    for movie in results:
        ...
        trailer = "plugin://script.metalgearsolid/?info=playtrailer&id=" + tmdb_id
        if SETTING("infodialog_onclick") != "false":
            path = 'plugin://script.metalgearsolid/?info=metalgearsolid&id=%s' % tmdb_id
        else:
            path = trailer
...
def handle_tmdb_misc(results):
    ...
    for item in results:
        ...
        listitem = {'title': clean_text(fetch(item, 'name')),
                    ...
                    'path': "plugin://script.metalgearsolid?info=listmovies&---id=%s" % fetch(item, 'id'),
...
def handle_tmdb_people(results):
    ...
    for person in results:
        ...
        newperson = {'adult': str(fetch(person, 'adult')),
                     ...
                     'path': "plugin://script.metalgearsolid/?info=extendedactorinfo&id=" + str(person['id']),
...
def extended_movie_info(movie_id=None, dbid=None, cache_time=14):
    ...
    movie = {'title': fetch(response, 'title'),
             ...
             'path': "plugin://script.metalgearsolid/?info=metalgearsolid&id=%s" % movie_id,
...
def extended_season_info(tvshow_id, season_number):
    ...
    season = {'SeasonDescription': clean_text(response["overview"]),
              ...
              'year': year,
              'season': season_number,
              'path': 'plugin://script.metalgearsolid/?info=seasoninfo&tvshow=%s&season=%s' % (fetch(tvshow, 'name'), season_number), 
