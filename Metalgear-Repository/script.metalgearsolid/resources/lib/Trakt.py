...
TRAKT_KEY = '54bc93a93f119d8ced87f9d708f5731e551d0c9f071f9b105a00c42703de92aa'
        ...
        for episode in day[1]:
            if episode["show"]["ids"]["tvdb"] or episode["show"]["ids"]["tmdb"] or episode["show"]["ids"]["imdb"] or episode["show"]["ids"]["tvrage"]:
                ...
                show = {'title': "%s - %dx%d - %s" % (episode["show"]["title"], episode["episode"]["season"], episode["episode"]["number"], episode["episode"]["title"]),
                        ...
                        'path': 'plugin://script.metalgearsolid/?info=extendedtvinfo&tvdb_id=%s' % episode["show"]["ids"]["tvdb"],
...
def handle_trakt_movies(results):
    ...
    for movie in results:
        ...
        movie = {'title': movie["movie"]["title"],
                 ...
                 'Trailer': 'plugin://script.metalgearsolid/?info=playtrailer&id=%s' % str(movie["movie"]["ids"]["tmdb"]),
                 'year': movie["movie"]["year"],
                 'id': id,
                 'imdb_id': movie["movie"]["ids"]["imdb"],
                 'trakt_id': movie["movie"]["ids"]["trakt"],
                 'slug': movie["movie"]["ids"]["slug"],
                 'path': 'plugin://script.metalgearsolid/?info=metalgearsolid&id=%s' % str(movie["movie"]["ids"]["tmdb"]), 
...
def handle_trakt_tvshows(results):
    ...
    for tvshow in results:
        ...
        show = {'title': tvshow['show']["title"],
                ...
                'Trailer': 'plugin://script.metalgearsolid/?info=playtvtrailer&id=%s' % str(tvshow['show']['ids']["tmdb"]),
                ...
                'path': 'plugin://script.metalgearsolid/?info=extendedtvinfo&id=%s' % str(tvshow['show']['ids']["tmdb"]),
