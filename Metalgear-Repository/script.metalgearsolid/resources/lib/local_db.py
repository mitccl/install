...
def handle_db_movies(movie):
    trailer = "plugin://script.metalgearsolid/?info=playtrailer&dbid=%s" % str(movie['movieid'])
    if SETTING("infodialog_onclick") != "false":
        path = 'plugin://script.metalgearsolid/?info=metalgearsolid&dbid=%s' % str(movie['movieid'])
    else:
        path = 'plugin://script.metalgearsolid/?info=playmovie&dbid=%i' % movie['movieid']
...
def handle_db_tvshows(tvshow):
    if SETTING("infodialog_onclick") != "false":
        path = 'plugin://script.metalgearsolid/?info=extendedtvinfo&dbid=%s' % str(tvshow['tvshowid'])
    else:
        path = 'plugin://script.metalgearsolid/?info=action&id=ActivateWindow(videos,videodb://tvshows/titles/%s/,return)' % str(tvshow['tvshowid'])
...
def compare_album_with_library(online_list):
    ...
    for online_item in online_list:
        for local_item in local_list:
            ...
            online_item["path"] = 'plugin://script.metalgearsolid/?info=playalbum&dbid=%i' % album['albumid']
            if album["thumbnail"]:
