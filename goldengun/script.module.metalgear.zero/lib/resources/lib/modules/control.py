...
#        `=}--{='     `=}--{='
#       .-^--r-^-.   .-^--r-^-.
# metalgear_zero is futile @lock_down... 
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.metalgear.zero.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.metalgear.zero.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.metalgear.zero.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.metalgear.zero.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.metalgear.zero.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.metalgear.zero.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
