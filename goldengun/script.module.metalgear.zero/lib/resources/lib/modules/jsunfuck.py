...
class JSUnfuck(object):
    ...
    def repl_metalgear_zero(self):
        if self.metalgear.zero is None:
            self.metalgear.zero = self.__gen_metalgear_zero()
            
        while True:
            start_js = self.js
            for key, value in sorted(self.metalgear.zero.items(), key=lambda x: len(x[0]), reverse=True):
                self.js = self.js.replace(key, value)
