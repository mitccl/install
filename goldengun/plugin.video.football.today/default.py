...
class videos:
    ...
    def root(self, url):
        ...
        #if '/highlights/' in url: self.list = self.lfv_list2(url)
        #else: self.list = self.lfv_list(url)
...
class videoparts:
        ...
        for video in videos:
            ...
            try:
                ...
                if 'proxy.link=lfv*' in video:
                    ...
                    parts = [i for i in parts if (HostedMediaFile(host=i, media_id="dummy").valid_url() or any(i.startswith(x) for x in resolver().hostList))]
                else:
                    video = video.replace('"//', '"http://').replace("'//", '"http://')
                    parts = re.findall('[\'|\"](https?://.+?)[\'|\"]', video, re.I)
                    #parts = [i for i in parts if any(i.startswith(x) for x in resolver().hostList)]
                    parts = [i for i in parts if (HostedMediaFile(host=i, media_id="dummy").valid_url() or any(i.startswith(x) for x in resolver().hostList))]
                ...
                for url in parts:
                    ...
                    if not (HostedMediaFile(host=i, media_id="dummy").valid_url() or any(i.startswith(x) for x in resolver().hostList)): continue
                    url = url.encode('utf-8')
    ...
    def lfv_list2(self, url, meta):
        try:
            ...
            videos = re.findall('[\'|\"](https?://.+?)[\'|\"]', videos, re.I)
            videos = uniqueList(videos).list
            #videos = [i for i in videos if any(i.startswith(x) for x in resolver().hostList)]
            videos = [i for i in videos if (HostedMediaFile(host=i, media_id="dummy").valid_url() or any(i.startswith(x) for x in resolver().hostList))]
...
class resolver:
    def __init__(self):
        ...
        self.footballfullmatch_base = 'http://player.footballfullmatch.com'
        self.hostList = self.host_list()

    def run(self, url):
        try:
            #print url
            if url.startswith(self.footballfullmatch_base): url = self.footballfullmatch(url)
            #elif url.startswith(self.vk_base): url = self.vk(url)
    ...
    def host_list(self):
        return [self.footballfullmatch_base]
    ...
    def footballfullmatch(self, url):
        try:
            result = getUrl(url).result
            result = common.parseDOM(result, "div", attrs = { "id": "fullheight" })[0]
            result = re.search('({.*})', result).groups()[0]
            result = result.replace("'", '"')
            sources = json.loads(result)["sources"]
            
            if len(sources) > 1:
                sources = sorted(sources, key=lambda k: k["label"], reverse=True)
                mirrors = list()
                for i in sources:
                    mirrors.extend([i["label"]])
            
                dialog = xbmcgui.Dialog()
                src = dialog.select('Select Quality', mirrors)
                
                if src == -1: return
                return sources[src]["file"]
                
            else: return sources[0]["file"]
        except:
            return
