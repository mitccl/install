...
class WindowManager(object):
	...
	def open_movie_info(self, prev_window=None, movie_id=None, dbid=None, name=None, imdb_id=None):
		...
		if Utils.NETFLIX_VIEW == 'true':
			dialog = movieclass(u'script.metalgearsolid-DialogVideoInfo-Netflix.xml', Utils.ADDON_PATH, id=movie_id, dbid=dbid)
			...
			if Utils.SKIN_DIR == 'skin.estuary':
				dialog = movieclass(u'script.metalgearsolid-DialogVideoInfo-Estuary.xml', Utils.ADDON_PATH, id=movie_id, dbid=dbid)
			elif Utils.SKIN_DIR == 'skin.aura' or 'skin.auramod':
				dialog = movieclass(u'script.metalgearsolid-DialogVideoInfo-Aura.xml', Utils.ADDON_PATH, id=movie_id, dbid=dbid)
			else:
				dialog = movieclass(u'script.metalgearsolid-DialogVideoInfo.xml', Utils.ADDON_PATH, id=movie_id, dbid=dbid)
		Utils.hide_busy()
	...
	def open_tvshow_info(self, prev_window=None, tmdb_id=None, dbid=None, tvdb_id=None, imdb_id=None, name=None):
		...
		if Utils.NETFLIX_VIEW == 'true':
			dialog = tvshow_class(u'script.metalgearsolid-DialogVideoInfo-Netflix.xml', Utils.ADDON_PATH, tmdb_id=tmdb_id, dbid=dbid)
			if Utils.AUTOPLAY_TRAILER == 'true' and not xbmc.getCondVisibility('VideoPlayer.IsFullscreen') and not xbmc.Player().isPlayingAudio():
				play_tv_trailer(tmdb_id)
		else:
			if Utils.SKIN_DIR == 'skin.estuary':
				dialog = tvshow_class(u'script.metalgearsolid-DialogVideoInfo-Estuary.xml', Utils.ADDON_PATH, tmdb_id=tmdb_id, dbid=dbid)
			elif Utils.SKIN_DIR == 'skin.aura' or 'skin.auramod':
				dialog = tvshow_class(u'script.metalgearsolid-DialogVideoInfo-Aura.xml', Utils.ADDON_PATH, tmdb_id=tmdb_id, dbid=dbid)
			else:
				dialog = tvshow_class(u'script.metalgearsolid-DialogVideoInfo.xml', Utils.ADDON_PATH, tmdb_id=tmdb_id, dbid=dbid)
		Utils.hide_busy()
		self.open_dialog(dialog, prev_window)
	...
	def open_season_info(self, prev_window=None, tvshow_id=None, season=None, tvshow=None, dbid=None):
		...
		if Utils.NETFLIX_VIEW == 'true':
			dialog = season_class(u'script.metalgearsolid-DialogVideoInfo-Netflix.xml', Utils.ADDON_PATH, tvshow_id=tvshow_id, season=season, dbid=dbid)
		else:
			if Utils.SKIN_DIR == 'skin.estuary':
				dialog = season_class(u'script.metalgearsolid-DialogVideoInfo-Estuary.xml', Utils.ADDON_PATH, tvshow_id=tvshow_id, season=season, dbid=dbid)
			elif Utils.SKIN_DIR == 'skin.aura' or 'skin.auramod':
				dialog = season_class(u'script.metalgearsolid-DialogVideoInfo-Aura.xml', Utils.ADDON_PATH, tvshow_id=tvshow_id, season=season, dbid=dbid)
			else:
				dialog = season_class(u'script.metalgearsolid-DialogVideoInfo.xml', Utils.ADDON_PATH, tvshow_id=tvshow_id, season=season, dbid=dbid)
		Utils.hide_busy()
		self.open_dialog(dialog, prev_window)
	...
	def open_episode_info(self, prev_window=None, tvshow_id=None, tvdb_id=None, season=None, episode=None, tvshow=None, dbid=None):
		...
		if Utils.NETFLIX_VIEW == 'true':
			dialog = ep_class(u'script.metalgearsolid-DialogVideoInfo-Netflix.xml', Utils.ADDON_PATH, tvshow_id=tvshow_id, season=season, episode=episode, dbid=dbid)
		else:
			if Utils.SKIN_DIR == 'skin.estuary':
				dialog = ep_class(u'script.metalgearsolid-DialogVideoInfo-Estuary.xml', Utils.ADDON_PATH, tvshow_id=tvshow_id, season=season, episode=episode, dbid=dbid)
			elif Utils.SKIN_DIR == 'skin.aura' or 'skin.auramod':
				dialog = ep_class(u'script.metalgearsolid-DialogVideoInfo-Aura.xml', Utils.ADDON_PATH, tvshow_id=tvshow_id, season=season, episode=episode, dbid=dbid)
			else:
				dialog = ep_class(u'script.metalgearsolid-DialogVideoInfo.xml', Utils.ADDON_PATH, tvshow_id=tvshow_id, season=season, episode=episode, dbid=dbid)
		Utils.hide_busy()
		self.open_dialog(dialog, prev_window)
	...
	def open_actor_info(self, prev_window=None, actor_id=None, name=None):
		...
		if Utils.SKIN_DIR == 'skin.estuary':
			dialog = actor_class(u'script.metalgearsolid-DialogInfo-Estuary.xml', Utils.ADDON_PATH, id=actor_id)
		elif Utils.SKIN_DIR == 'skin.aura' or 'skin.auramod':
			dialog = actor_class(u'script.metalgearsolid-DialogInfo-Aura.xml', Utils.ADDON_PATH, id=actor_id)
		else:
			dialog = actor_class(u'script.metalgearsolid-DialogInfo.xml', Utils.ADDON_PATH, id=actor_id)
		Utils.hide_busy()
		self.open_dialog(dialog, prev_window)
	...
	def open_video_list(self, prev_window=None, listitems=None, filters=[], mode='filter', list_id=False, filter_label='', media_type='movie', search_str=''):
		...
		if Utils.NETFLIX_VIEW == 'true':
			dialog = browser_class(u'script.metalgearsolid-VideoList-Netflix.xml', Utils.ADDON_PATH, listitems=listitems, filters=filters, mode=mode, list_id=list_id, filter_label=filter_label, type=media_type, search_str=search_str)
		else:
			if Utils.SKIN_DIR == 'skin.estuary':
				dialog = browser_class(u'script.metalgearsolid-VideoList-Estuary.xml', Utils.ADDON_PATH, listitems=listitems, filters=filters, mode=mode, list_id=list_id, filter_label=filter_label, type=media_type, search_str=search_str)
			elif Utils.SKIN_DIR == 'skin.aura' or 'skin.auramod':
				dialog = browser_class(u'script.metalgearsolid-VideoList-Aura.xml', Utils.ADDON_PATH, listitems=listitems, filters=filters, mode=mode, list_id=list_id, filter_label=filter_label, type=media_type, search_str=search_str)
			else:
				dialog = browser_class(u'script.metalgearsolid-VideoList.xml', Utils.ADDON_PATH, listitems=listitems, filters=filters, mode=mode, list_id=list_id, filter_label=filter_label, type=media_type, search_str=search_str)
		if prev_window:
	...
	def open_slideshow(self, listitems, index):
		if Utils.SKIN_DIR == 'skin.estuary':
			slideshow = SlideShow(u'script.metalgearsolid-SlideShow-Estuary.xml', Utils.ADDON_PATH, listitems=listitems, index=index)
		elif Utils.SKIN_DIR == 'skin.aura' or 'skin.auramod':
			slideshow = SlideShow(u'script.metalgearsolid-SlideShow-Aura.xml', Utils.ADDON_PATH, listitems=listitems, index=index)
		else:
			slideshow = SlideShow(u'script.metalgearsolid-SlideShow.xml', Utils.ADDON_PATH, listitems=listitems, index=index)
		slideshow.doModal()
