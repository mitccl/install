	...
	class DialogVideoInfo(DialogBaseInfo, window_type):
		...
		def play_movie(self):
			if self.dbid and int(self.dbid) > 0:
				...
				url = 'plugin://plugin.video.metalgear/movies/play/tmdb/%s' % self.info.get('id', '')
		...
		def play_movie_choose_player(self):
			if self.dbid and int(self.dbid) > 0:
				...
				url = 'plugin://plugin.video.metalgear/movies/play_choose_player/tmdb/%s/False' % self.info.get('id', '')
				xbmc.executebuiltin('RunPlugin(%s)' % url)

		@ch.click(445)
		def show_manage_dialog(self):
			manage_list = []
			manage_list.append(["Metalgearsolid's settings", 'Addon.OpenSettings("script.metalgearsolid")'])
			manage_list.append(["metalgear's settings", 'Addon.OpenSettings("plugin.video.metalgear")'])
		...
		def add_movie_to_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.metalgear)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.metalgear/setup/total)')
			if xbmcgui.Dialog().yesno('Metalgearsolid', 'Add [B]%s[/B] to library?' % self.info['title']):
		...
		def remove_movie_from_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.metalgear)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.metalgear/setup/total)')
			if xbmcgui.Dialog().yesno('Metalgearsolid', 'Remove [B]%s[/B] from library?' % self.info['title']):
