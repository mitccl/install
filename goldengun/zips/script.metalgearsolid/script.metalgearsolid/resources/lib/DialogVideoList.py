...
def get_tmdb_window(window_type):
	...
	class DialogVideoList(DialogBaseList, window_type):
		...
		def context_menu(self):
			...
			if selection == 0:
				if self.listitem.getProperty('TVShowTitle'):
					url = 'plugin://plugin.video.metalgear/tv/play/%s/1/1' % tvdb_id
					...
					if self.listitem.getProperty('dbid'):
						...
						dbid = 0
						url = 'plugin://plugin.video.metalgear/movies/play/tmdb/%s' % item_id
					PLAYER.play_from_button(url, listitem=None, window=self, type='movieid', dbid=dbid)
			if selection == 1:
				if self.listitem.getProperty('TVShowTitle'):
					TVLibrary = xbmcaddon.Addon('plugin.video.metalgear').getSetting('tv_library_folder')
					if self.listitem.getProperty('dbid'):
						...
						if xbmcgui.Dialog().yesno('Metalgearsolid', 'Add [B]%s[/B] to library?' % self.listitem.getProperty('TVShowTitle')):
							xbmc.executebuiltin('RunPlugin(plugin://plugin.video.metalgear/tv/add_to_library/%s)' % tvdb_id)
							Utils.after_add(type='tv')
					...
					if self.listitem.getProperty('dbid'):
						if xbmcgui.Dialog().yesno('Metalgearsolid', 'Remove [B]%s[/B] from library?' % self.listitem.getProperty('title')):
							Utils.get_kodi_json(method='VideoLibrary.RemoveMovie', params='{"movieid": %s}' % dbid)
							MovieLibrary = xbmcaddon.Addon('plugin.video.metalgear').getSetting('movies_library_folder')
						...
						if xbmcgui.Dialog().yesno('Metalgearsolid', 'Add [B]%s[/B] to library?' % self.listitem.getProperty('title')):
							xbmc.executebuiltin('RunPlugin(plugin://plugin.video.metalgear/movies/add_to_library/tmdb/%s)' % item_id)
							Utils.after_add(type='movie')
			...
			if selection == 2:
				if self.listitem.getProperty('TVShowTitle'):
					url = 'plugin://script.metalgearsolid?info=playtvtrailer&&id=' + item_id
				else:
					url = 'plugin://script.metalgearsolid?info=playtrailer&&id=' + item_id
