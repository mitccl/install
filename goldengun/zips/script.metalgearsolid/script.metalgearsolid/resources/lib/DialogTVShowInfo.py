...
def get_tvshow_window(window_type):
	...
	class DialogTVShowInfo(DialogBaseInfo, window_type):
		...
		def browse_tvshow(self):
			url = 'plugin://plugin.video.metalgear/tv/tvdb/%s/' % self.info['tvdb_id']
			self.close()
		...
		def play_tvshow(self):
			url = 'plugin://plugin.video.metalgear/tv/play/%s/1/1' % self.info['tvdb_id']
			xbmc.executebuiltin('RunPlugin(%s)' % url)

		@ch.action('contextmenu', 9)
		def play_tvshow_choose_player(self):
			url = 'plugin://plugin.video.metalgear/tv/play_choose_player/%s/1/1/False' % self.info['tvdb_id']
			xbmc.executebuiltin('RunPlugin(%s)' % url)
		...
		def add_tvshow_to_library(self):
			...
			if xbmcgui.Dialog().yesno('Metalgearsolid', 'Add [B]%s[/B] to library?' % self.info['TVShowTitle']):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.metalgear/tv/add_to_library/%s)' % self.info['tvdb_id'])
		...
		def remove_tvshow_from_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.metalgear)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.metalgear/setup/total)')
			if xbmcgui.Dialog().yesno('Metalgearsolid', 'Remove [B]%s[/B] from library?' % self.info['TVShowTitle']):
