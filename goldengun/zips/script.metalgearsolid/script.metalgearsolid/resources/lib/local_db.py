...
def handle_db_movies(movie):
	trailer = 'plugin://script.metalgearsolid?info=playtrailer&&dbid=%s' % movie['movieid']
	path = 'plugin://script.metalgearsolid?info=extendedinfo&&dbid=%s' % movie['movieid']
...
def handle_db_tvshows(tvshow):
	path = 'plugin://script.metalgearsolid?info=extendedtvinfo&&dbid=%s' % tvshow['tvshowid']
	db_tvshow = {
		'mediatype': 'tvshow',
