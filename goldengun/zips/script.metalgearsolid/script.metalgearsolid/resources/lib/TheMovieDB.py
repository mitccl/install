...
def handle_tmdb_movies(results=[], local_first=True, sortkey='year'):
	...
	for movie in results:
		...
		trailer = 'plugin://script.metalgearsolid?info=playtrailer&&id=%s' % tmdb_id
		path = 'plugin://script.metalgearsolid?info=extendedinfo&&id=%s' % tmdb_id
		listitem = {
...
def handle_tmdb_tvshows(results, local_first=True, sortkey='year'):
	...
	for tv in results:
		...
		newtv = {
			...
			'Trailer': 'plugin://script.metalgearsolid?info=tvtrailer&&id=%s' % tmdb_id,
			'year': Utils.get_year(Utils.fetch(tv, 'first_air_date')),
			'media_type': 'tv',
			'mediatype': 'tvshow',
			'character': Utils.fetch(tv, 'character'),
			'path': 'plugin://script.metalgearsolid?info=extendedtvinfo&&id=%s' % tmdb_id,
...
def handle_tmdb_misc(results):
	...
	for item in results:
		...
		listitem = {
			...
			'path': 'plugin://script.metalgearsolid?info=listmovies&---id=%s' % Utils.fetch(item, 'id'),
...
def handle_tmdb_people(results):
	...
	for person in results:
		...
		newperson = {
			...
			'path': 'plugin://script.metalgearsolid?info=extendedactorinfo&&id=%s' % str(person['id']),
...
def extended_movie_info(movie_id=None, dbid=None, cache_time=14):
	...
	movie = {
		...
		'path': 'plugin://script.metalgearsolid?info=extendedinfo&&id=%s' % movie_id,
		'trailer': 'plugin://script.metalgearsolid?info=playtrailer&&id=%s' % movie_id
		}
...
def extended_tvshow_info(tvshow_id=None, cache_time=7, dbid=None):
	...
	tvshow = {
		...
		'Votes': Utils.fetch(response, 'vote_count'),
		'Status': translate_status(Utils.fetch(response, 'status')),
		'path': 'plugin://script.metalgearsolid?info=extendedtvinfo&&id=%s' % tvshow_id,
		'trailer': 'plugin://script.metalgearsolid?info=playtvtrailer&&id=%s' % tvshow_id,
...
def extended_season_info(tvshow_id, season_number):
	...
	season = {
		...
		'year': year,
		'season': season_number,
		'path': 'plugin://script.metalgearsolid?info=seasoninfo&tvshow=%s&season=%s' % (Utils.fetch(tvshow, 'name'), season_number),
...
def extended_episode_info(tvshow_id, season, episode, cache_time=7):
	...
	answer = {
		...
		'tvdb_id': tvdb_id,
		'actors': actors,
		'path': 'plugin://script.metalgearsolid?info=extendedepisodeinfo&tvshow_id=%s&season=%s&episode=%s' % (tvshow_id, season, episode),
		'crew': crew,
