...
import urlparse
from resistancescrapers.modules import control
from resistancescrapers import providerSources, providerNames

params = dict(urlparse.parse_qsl(sys.argv[2].replace('?', '')))
mode = params.get('mode')

def ScraperChoice():
    from resistancescrapers import providerSources
...
def ToggleProviderAll(enable):
    from resistancescrapers import providerNames
...
def toggleAll(setting, open_id=None, sourceList=None):
    from resistancescrapers import getAllHosters
...
if mode == "resistanceSettings":
    control.openSettings('0.0', 'script.module.resistancescrapers')
