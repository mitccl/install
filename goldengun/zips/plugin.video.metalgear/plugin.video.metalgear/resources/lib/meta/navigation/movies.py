...
def movies_batch_add_to_library():
    ...
    if xbmcvfs.exists(movie_batch_file):
        ...
        if xbmcvfs.exists(plugin.get_setting(SETTING_TV_BATCH_ADD_FILE_PATH, unicode)):
            xbmc.executebuiltin("RunPlugin(plugin://plugin.video.metalgear/tv/batch_add_to_library)")
            return True
        else:
            xbmc.sleep(1000)
            ...
            scan_library(type="video", path=plugin.get_setting(SETTING_MOVIES_LIBRARY_FOLDER, unicode))
            return True
    if xbmcvfs.exists(plugin.get_setting(SETTING_TV_BATCH_ADD_FILE_PATH, unicode)): xbmc.executebuiltin("RunPlugin(plugin://plugin.video.metalgear/tv/batch_add_to_library)")
