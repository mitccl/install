...
class tvshows:
    ...
    def browse(self):
        ...
        match = re.compile('<a href="/search/keyword\?keywords=(.+?)&.+?" > (.+?) </a>').findall(r)
    ...
    def findKeywords(self, url):
        ...
        sysaddon = sys.argv[0]
        cm = []
        cm.append(('Browse tags test', 'ActivateWindow(10025,%s?action=findKeywordsTV&url=me,return)'  % sysaddon))
        self.addDirectory(self.list)
        return self.list
    ...
    def tvshowDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                cm.append(('Jor-EL Settings', 'RunPlugin(%s?action=openSettings&query=(0,0))' % sysaddon))
