...
class source:
	def __init__(self):
		...
		privateToken = xbmcaddon.Addon('plugin.video.covenant').getSetting('private.rd.enable')
		if privateToken == 'true':
			self.token = xbmcaddon.Addon('plugin.video.covenant').getSetting('private.rd.api')
		else:
