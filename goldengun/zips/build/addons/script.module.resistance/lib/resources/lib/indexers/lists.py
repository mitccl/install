...
import base64,hashlib,os,random,re,requests,shutil,string,sys,urllib,urllib2,json,resolveurl,ssl,zipfile,urlparse, datetime
import xbmc,xbmcgui
...
addon_id = 'plugin.video.resistance'
...
class indexer:
    ...
    def root(self):#
        try:
            regex.clear()
            url = 'https://raw.githubusercontent.com/Goldengunrepo/install/master/goldengun/xml/main.xml'
            self.list = self.resistance_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.resistance_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.resistance_list('', result=result)
    ...
    def addSearch(self, url):
            ...
            link = 'http://185.22.154.90/solid_builds/resistance/1clicks.xml'
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.resistance_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['resistance TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def resistance_list(self, url, result=None):
