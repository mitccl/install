...
class movies:
    def __init__(self):
		...
		self.twentytwo_link = 'https://www.imdb.com/search/title/?title_type=feature&release_date=2020-01-01,2029-12-31'
...
#		self._link = 'http://www.imdb.com/search/title?,desc&count=40&start=1'
    ...
    def years(self):
		...
		for i in range(int(year)-0, 1900, -1): self.list.append({'name': str(i), 'url': self.year_link % (str(i), str(i)), 'image': 'years.jpg', 'action': 'movies'})
		self.addDirectory(self.list)
		return self.list
