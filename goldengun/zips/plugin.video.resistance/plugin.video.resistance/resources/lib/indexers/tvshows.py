...
class tvshows:
    def __init__(self):
		...
		self.clsstv_link = 'https://www.imdb.com/search/title?title_type=tv_series&release_date=1900-01-01,1993-12-31&sort=moviemeter,asc'
		...
		self.bfsim_link = 'https://www.imdb.com/search/title/?genres=family&title_type=tv_series&sort=moviemeter,asc'
...
#		('Test', '/webchannels/45/ufc-fight-pass'),
		...
		for i in networks: self.list.append({'name': i[0], 'url': self.tvmaze_link + i[1], 'image': 'networks.jpg', 'action': 'tvshows'})
		self.addDirectory(self.list)
		return self.list
