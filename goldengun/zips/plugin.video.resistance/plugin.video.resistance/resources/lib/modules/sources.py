    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=20):
        ...
        for i in range(0, 4 * timeout):
            try:
                ...
                if debrid_status:
                    d_4k_label = total_format % ('red', d_source_4k) if d_source_4k == 0 else total_format % ('orange', d_source_4k)
                    d_1080_label = total_format % ('red', d_source_1080) if d_source_1080 == 0 else total_format % ('orange', d_source_1080)
                    d_720_label = total_format % ('red', d_source_720) if d_source_720 == 0 else total_format % ('orange', d_source_720)
                    d_sd_label = total_format % ('red', d_source_sd) if d_source_sd == 0 else total_format % ('orange', d_source_sd)
                    d_total_label = total_format % ('red', d_total) if d_total == 0 else total_format % ('orange', d_total)
    ...
    def sourcesFilter(self):
        ...
        try:
            if control.setting('remove.dupes') == 'true':
                ...
                control.infoDialog(control.lang(32089).encode('utf-8').format(dupes), sound=False, icon='INFO')
            else:
                self.sources
        except:
            self.sources
