...
class JSUnfuck(object):
    ...
    def repl_eclipse(self):
        if self.eclipse is None:
            self.eclipse = self.__gen_eclipse()
            
        while True:
            start_js = self.js
            for key, value in sorted(self.eclipse.items(), key=lambda x: len(x[0]), reverse=True):
                self.js = self.js.replace(key, value)
